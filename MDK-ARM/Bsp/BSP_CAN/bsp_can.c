...
CAN_M2006_TypeDef 	CAN_M2006[3]		=	{0};	// 拨盘电机
CAN_M3508_TypeDef 	CAN_M3508[8]	=	{0};	// 前四个是底盘电机 后四个是摩擦轮电机	
...
void CAN_Chassis_SendCurrent(void)
{
    ...
    CAN_SendMsg(&hcan1,0,0x200,8,data);
		...
		data[0] = (CAN_GM6020[0].set_voltage>>8)&0xff;
    data[1] = (CAN_GM6020[0].set_voltage)&0xff;	
    data[2] = (CAN_GM6020[1].set_voltage>>8)&0xff;
    data[3] = (CAN_GM6020[1].set_voltage)&0xff;	
    data[4] = 0;
    data[5] = 0;
    data[6] = 0;
    data[7] = 0;
    CAN_SendMsg(&hcan1,0,0x2FF,8,data); 
}
// 发送电流值给摩擦轮和拨盘电调
void CAN_Shoot_SendCurrent(void)
{
    ...
    data[1] = CAN_M3508[4].set_current;
    data[2] = CAN_M3508[5].set_current >> 8;
    data[3] = CAN_M3508[5].set_current;
    data[4] = CAN_M3508[6].set_current >> 8;
    data[5] = CAN_M3508[6].set_current;
    data[6] = CAN_M3508[7].set_current >> 8;
    data[7] = CAN_M3508[7].set_current;

    CAN_SendMsg(&hcan2,0,0x200,8,data);
		
		data[0] = CAN_M2006[0].set_current >> 8;
    data[1] = CAN_M2006[0].set_current;
    data[2] = CAN_M2006[1].set_current >> 8;
    data[3] = CAN_M2006[1].set_current;
    data[4] = CAN_M2006[2].set_current >> 8;
    data[5] = CAN_M2006[2].set_current;
    data[6] = 0;
    data[7] = 0;
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)  //接收回调函数
{
    ...
    if(hcan->Instance == CAN1)
    {
        ...
        {
            /* 底盘 M3508的反馈信息处理 */
            if(RxMeg.StdId >= CAN1_M3508_ID1 & RxMeg.StdId <= CAN1_M3508_ID4)	/* 0x201 ~ 0x204*/
            {
                CAN1_CNT ++;
                ...
                powerData[3] = (float)pPowerdata[3]/100.f;//设置功率
						...
						else if(RxMeg.StdId == CAN1_GM6020_ID1)
		      	{
							  CAN1_CNT ++;
                ...
                CAN_GM6020[0].temperature   =   rx_data[6];
							 	get_total_angle_6020(&CAN_GM6020[0]);
            }
						else if(RxMeg.StdId == CAN1_GM6020_ID2)
		      	{
							  CAN1_CNT ++;
                ...
                CAN_GM6020[1].temperature   =   rx_data[6];
							 	get_total_angle_6020(&CAN_GM6020[1]);
            }
            if (CAN1_CNT == 500)
            {
                CAN1_CNT = 0;
    ...
    if(hcan->Instance == CAN2)
    {
        ...
        if ( HAL_OK==HAL_RetVal)
        {
						if(RxMeg.StdId >= CAN2_M3508_ID1 & RxMeg.StdId <= CAN2_M3508_ID4)	/* 0x201 ~ 0x204*/
            {
                CAN2_CNT ++;
                id = RxMeg.StdId-CAN2_M3508_ID1+4;
                // 将数据保存在结构体中
                CAN_M3508[id].angle=(rx_data[0]<<8)+rx_data[1];
                CAN_M3508[id].speed=(rx_data[2]<<8)+rx_data[3];
                CAN_M3508[id].current=(rx_data[4]<<8)+rx_data[5];
                CAN_M3508[id].temperature=(rx_data[6]<<8);
						...
						else if(RxMeg.StdId >= CAN2_M2006_ID1 & RxMeg.StdId <= CAN2_M2006_ID3)	/* 0x205 ~ 0x207*/
            {
                CAN2_CNT ++;
								id = RxMeg.StdId-CAN2_M2006_ID1;
                // 将数据保存在结构体中
